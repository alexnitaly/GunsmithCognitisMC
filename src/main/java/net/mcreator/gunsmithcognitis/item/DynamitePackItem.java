	...
	public void releaseUsing(ItemStack itemstack, Level world, LivingEntity entityLiving, int timeLeft) {
		if (!world.isClientSide() && entityLiving instanceof ServerPlayer entity) {
			...
			if (HandBombCanUseRangedItemProcedure.execute(entity)) {
				ItemStack stack = ProjectileWeaponItem.getHeldProjectile(entity, e -> e.getItem() == GunsmithCognitisModItems.DYNAMITE_PACK.get());
				if (stack == ItemStack.EMPTY) {
					for (int i = 0; i < entity.getInventory().items.size(); i++) {
						ItemStack teststack = entity.getInventory().items.get(i);
						if (teststack != null && teststack.getItem() == GunsmithCognitisModItems.DYNAMITE_PACK.get()) {
							stack = teststack;
							break;
						}
					}
				...
				if (entity.getAbilities().instabuild || stack != ItemStack.EMPTY) {
					...
					if (entity.getAbilities().instabuild) {
						...
						if (new ItemStack(GunsmithCognitisModItems.DYNAMITE_PACK.get()).isDamageableItem()) {
